 ...
 */
#define __HAL_API_VER_PATCH 0x17
...
typedef enum {
    ...
    MEM_TYPE_TS_LOG,
    MEM_TYPE_HBOOT_SRAM,
    MEM_TYPE_DEBUG_OS_LOG,          // slog/dev-os-id/debug/device-os
    MEM_TYPE_SEC_LOG,               // slog/dev-os-id/security
    MEM_TYPE_RUN_OS_LOG,            // slog/dev-os-id/run/device-os
    MEM_TYPE_RUN_EVENT_LOG,         // slog/dev-os-id/run/event
    MEM_TYPE_DEBUG_DEV_LOG,         // slog/dev-os-id/debug/device-id
...
struct halFaultEventInfo {
    ...
    unsigned char os_id;
    unsigned char resv[HAL_MAX_EVENT_RESV_LENGTH]; /* os_id occupy 1byte, pid occupy 4byte, reserve 27byte */
};
...
* MODULE_TYPE_HOST_AICPU    |  INFO_TYPE_OCCUPY           |   host aicpu bitmap         | used in host   |
...
* @attention Only by calling interface halSupportFeature(devId, FEATURE_SVM_GET_USER_MALLOC_ATTR) first
...
* @attention Only support ONLINE scene: in scenarios where hccl is not used for a single process,
...
* @param [in] addr Currently unused, must be zero.
