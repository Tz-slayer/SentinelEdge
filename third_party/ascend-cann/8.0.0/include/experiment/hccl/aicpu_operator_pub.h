...
struct BatchSendRecvPair {
    uint8_t bsrSendTag[128];
    uint8_t bsrRecvTag[128];
    uint32_t Sendindex;
    uint32_t Recvindex;
};
using HcclOpIdentifier = struct HcclOpIdentifierDef {
    ...
    u32 streamId = 0xFFFFFFFF;
    BatchSendRecvPair bsrInfo;
};
...
constexpr u32 LINK_P2P_MAX_NUM = 8 * 2;
constexpr u32 RDMA_NOTIFY_MAX_NUM = 8 * 3;
constexpr u32 RDMA_QP_MAX_NUM = 8;
constexpr u32 LOCAL_STREAM_MAX_NUM = 19;
constexpr u32 LOCAL_NOTIFY_MAX_NUM = 38;
using RANK_TYPE = u32;
using TAG_TYPE = u32;
...
struct HcclLinkP2pV2 {
    MemDetails localMem[2];
    MemDetails remoteMem[2];
...
struct HcclLinkRoceV2 {
    MemDetails localMem[2];
    MemDetails remoteMem[2];
    u64 notifyValue;  // notify record的src地址value存储位置
    u32 notifyValueKey;
    HcclSignalInfo localNotifyList[RDMA_NOTIFY_MAX_NUM];
    u64 remoteNotifyList[RDMA_NOTIFY_MAX_NUM];   // notify的addr
    u32 remoteNotifyKey;
    s64 chipId;
    HcclQpInfoV2 QpInfo[RDMA_QP_MAX_NUM];   // 最多八个
};
