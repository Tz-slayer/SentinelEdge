 ...
 * @param [in] additionalOutput: bool 类型，用于指定Add的计算结果x是否输出。当为Fasle时，算子不会给xOut输出，xOut此时为脏数据。
 * @param [in] divMode: bool 类型，用于指定静态量化计算的算法。当为False时，静态量化计算计算使用乘法，反之亦然。仅仅在quantMode为"dynamic"时生效。
 * @param [in] y1Out:
 ...
 * @return aclnnStatus: 返回状态码。
 */
 ACLNN_API aclnnStatus aclnnAddLayerNormQuant(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                     aclrtStream stream);
