 ...
 *
 * 算子功能：Add + RmsNorm + DynamicQuant量化计算的融合算子，将加法的计算结果做层归一化计算后进行量化，
 ...
 * @param [in] x2:
 * 公式的输入`x2`，数据类型支持BFLOAT16、FLOAT16，shape维度要小于或等于8维。
 * 支持非连续的Tensor，数据格式支持ND。
 * @param [in] gamma:
 * 公式的输入`gamma`，数据类型支持BFLOAT16、FLOAT16
 * ，shape维度需要小于或等于8维。
 * 支持非连续的Tensor，数据格式支持ND。
 * @param [in] smoothScale1Optional:
 * 公式输入`smoothScale1`，数据类型支持BFLOAT16、FLOAT16，shape维度需要等于x1的最后一维。
 * 支持非连续的Tensor，数据格式支持ND。
 * @param [in] smoothScale2Optional:
 * 公式中的输入`smoothScale2Optional`，数据类型支持BFLOAT16、FLOAT16，shape维度需要等于x1的最后一维。
 * 支持非连续的Tensor，数据格式支持ND。
 * @param [in] epsilon: 支持double 类型，层归一化中用到的防止除0的参数。
 * @param [in] y1Out:
 * 公式中的输出 `y1`，数据类型支持INT8，shape需要与x1一致。
 * 支持非连续的Tensor，数据格式支持ND。
 * @param [in] y2Out:
 * 公式的输出`y2`，数据类型支持INT8，shape需要与x1一致。
 * 支持非连续的Tensor，数据格式支持ND。
 * @param [in] xOut:
 ...
 * 支持非连续的Tensor，数据格式支持ND。
 * @param [in] scale2Out:
 * 公式的输出`scale2`，数据类型支持FLOAT，shape需要与x1除最后一维一致。
 * 支持非连续的Tensor，数据格式支持ND。
 ...
 * @return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus aclnnAddRmsNormDynamicQuant(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                           aclrtStream stream);
