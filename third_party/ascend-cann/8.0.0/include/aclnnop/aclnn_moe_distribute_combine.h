 ...
 * @param [in] activationScale: 计算输入，Tensor，数据类型float32，必须为1维，数据格式支持ND, 预留参数，暂未使用，传空即可。
 * @param [in] weightScale: 计算输入，Tensor，数据类型float32，必须为2维，数据格式支持ND,该参数必须有。预留参数，暂未使用，传空即可。
